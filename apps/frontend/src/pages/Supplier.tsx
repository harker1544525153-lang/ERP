...
import { PlusOutlined, EditOutlined, DeleteOutlined, SearchOutlined, DownloadOutlined, ShoppingCartOutlined } from '@ant-design/icons';
import { motion } from 'framer-motion';
import { getSuppliers, createSupplier, updateSupplier, deleteSupplier } from '@/api/supplier';
import { getUserFromStorage } from '@/utils/storage';
type Supplier = { id: string; code: string; name: string; shortName: string; contact: string; phone: string; address: string; taxCode: string; tenantId: string };
...
function SupplierPage() {
  ...
  useEffect(() => {
    ...
    if (user.tenantId) {
      getSuppliers(user.tenantId).then(res => setData(res.data));
    }
  }, []);
  ...
  const handleSubmit = () => {
    form.validateFields().then(values => {
      ...
      if (editingId) {
        updateSupplier(editingId, data).then((response) => {
  ...
  const columns = [
    { title: '供应商编码', dataIndex: 'code', key: 'code', width: 100 },
  ...
  return (
    <div className="page-container">
      ...
      >
        ...
        <span className="text-gradient">供应商管理</span>
      </motion.div>
          ...
          <Form form={form} layout="vertical">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <Form.Item name="code" label="供应商编码" rules={[{ required: true, message: '请输入供应商编码' }]}>
                <Input placeholder="请输入供应商编码" style={{ borderRadius: 8 }} />
              </Form.Item>
              <Form.Item name="name" label="供应商名称" rules={[{ required: true, message: '请输入供应商名称' }]}>
                <Input placeholder="请输入供应商名称" style={{ borderRadius: 8 }} />
              </Form.Item>
